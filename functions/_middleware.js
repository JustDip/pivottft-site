...
//
// Per-entity pages use real data from _comps.json / _champions.json /
// _items.json (generated by gen-ssr-data.mjs from the PivotTFT source). Inert
// on GitHub Pages; active on Cloudflare Pages.

import COMPS     from './_comps.json';
import CHAMPIONS from './_champions.json';
...
const CHAMPION_BY_SLUG = new Map(CHAMPIONS.map(c => [c.slug, c]));
...
function metaForPath(pathname) {
  ...
  // Per-champion page — /champions/<slug>/
  if (seg === 'champions' && parts[1]) {
    const champ = CHAMPION_BY_SLUG.get(parts[1]);
    const name = champ ? champ.name : deslug(parts[1]);
    const traits = champ && champ.traits && champ.traits.length
      ? ` ${champ.traits.join(' / ')} ${champ.cost}-cost.`
      : '';
    const desc = clamp(
      `${name} — TFT Set 17 champion guide.${traits} Best items, traits, comp synergies and live win rate.`,
      160
    );
    return {
      title: `${name} — TFT Set 17 Champion${SUFFIX}`,
      desc,
      canonical: `${SITE}/champions/${parts[1]}/`,
    };
  }

  // Per-item page — /items/<slug>/
  if (seg === 'items' && parts[1]) {
    const item = ITEM_BY_SLUG.get(parts[1]);
    const name = item ? item.name : deslug(parts[1]);
    const flavor = item && item.stats
      ? clamp(`${name}: ${item.stats}`, 160)
      : clamp(`${name} — TFT Set 17 item: components, best holders, win rate and pick rate.`, 160);
    return {
      title: `${name} — TFT Set 17 Item${SUFFIX}`,
      desc: flavor,
      canonical: `${SITE}/items/${parts[1]}/`,
    };
  }
